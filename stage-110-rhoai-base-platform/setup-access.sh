...
oc get oauth cluster -o json | python3 - "$IDP_NAME" "$HTPASSWD_SECRET" <<'EOF' | oc apply -f -
import json, sys
name, secret = sys.argv[1], sys.argv[2]
...
oc adm groups add-users "$DEV_GROUP" "$DEV_USER"
...
if oc get auth.services.platform.opendatahub.io auth >/dev/null 2>&1; then
    oc get auth.services.platform.opendatahub.io auth -o json | python3 - "$ADMIN_GROUP" "$DEV_GROUP" <<'EOF' | oc apply -f -
import json, sys
admin_g, dev_g = sys.argv[1], sys.argv[2]
...
echo "=== Demo access setup complete. Users: $ADMIN_USER, $DEV_USER (passwords in .env) ==="
